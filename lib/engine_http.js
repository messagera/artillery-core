...
HttpEngine.prototype.step = function step(requestSpec, ee) {
  ...
  if (requestSpec.messageSequence) {
    let steps = _.map(requestSpec.messageSequence, function(rs) {
      return self.step(rs, ee);
    });

    return engineUtil.createMessageSequence(requestSpec.sequence, steps);
  }
