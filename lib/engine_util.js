...
module.exports = {
  ...
  createMessageSequence: createMessageSequence,
  template: template,
  evil: evil
};
...
function createMessageSequence(sequence, steps) {
  return function aLoop(context, callback) {
    
    let i = 0;
    let newContext = context;
    
    //newContext.vars.userId = sequence[i].message;
    A.whilst(
      function test() {
      ...
      function repeated(cb) {
        let zero = function(cb2) {
          newContext.vars.message = sequence[i].message;
          newContext.vars.delay = sequence[i].delay;
          return setTimeout(function(){cb2(null, newContext);}, (Math.random()* newContext.vars.delay))
        };
        ...
        A.waterfall(steps2, function(err, context2) {
          i++;
          newContext = context2;
          return setTimeout(function(){cb(err, context2);}, context2.vars.delay);
        });
      },
